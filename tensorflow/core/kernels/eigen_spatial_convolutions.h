...
EIGEN_DEVICE_FUNC
  ...
  TensorRef<Tensor<typename internal::traits<Kernel>::Scalar,
      ...
      kern(kernel);
  printf("SpatialConvolution called\n");
  EIGEN_STATIC_ASSERT(
  ...
  if(internal::traits<Input>::Layout == ColMajor){
    std::cout << "col major" << std::endl;
  }
  std::cout << "Params:" << kernelRows << " " <<kernelCols << " "
            << row_stride << " " << col_stride << " "
            << row_in_stride << " " << col_in_stride << " "
            << padding_type << " " << kernel_dims[0] << " "
            << kernel_dims[1] << " " << std::endl;

  // tensor.device(d) = input
  //     .extract_image_patches(kernelRows, kernelCols, row_stride, col_stride,
  //                            row_in_stride, col_in_stride, padding_type);
  std::cout << typeid(input
      .extract_image_patches(kernelRows, kernelCols, row_stride, col_stride,
                             row_in_stride, col_in_stride, padding_type)).name()
                             << std::endl;
 ...
 * that the same order is used in the input, the kernel, and the output.
  *
  */
template <typename Device, typename Input, typename Inter, typename Kernel>
EIGEN_DEVICE_FUNC
//     EIGEN_ALWAYS_INLINE static const typename internal::conditional<
//         internal::traits<Input>::Layout == ColMajor,
//         TensorReshapingOp<
...
//                     const DSizes<typename internal::traits<Input>::Index, 2>,
//                     const Kernel> > > >::type
EIGEN_ALWAYS_INLINE static const TensorReshapingOp<const Eigen::DSizes<long, 2>, const Eigen::TensorImagePatchOp<-1, -1, const Eigen::TensorMap<Eigen::Tensor<const float, 4, 1, long>, 16, MakePointer> > >
    SpatialConvolutionModified(const Device& device, const Inter& inter, const Input& input, const Kernel& kernel,
                       const DenseIndex row_stride = 1,
                       const DenseIndex col_stride = 1,
                       const PaddingType padding_type = PADDING_SAME,
                       const DenseIndex row_in_stride = 1,
  ...
  TensorRef<Tensor<typename internal::traits<Kernel>::Scalar,
                   ...
                   internal::traits<Kernel>::Layout, TensorIndex> >
      kern(kernel);
  printf("SpatialConvolutionModified called\n");
  EIGEN_STATIC_ASSERT(
      internal::traits<Input>::Layout == internal::traits<Kernel>::Layout,
      YOU_MADE_A_PROGRAMMING_MISTAKE);
  const bool isColMajor = (internal::traits<Input>::Layout == ColMajor);
  ...
  // TODO(yangke): choose() is defined in TensorContraction.h -- consider
  // moving it to somewhere more "common".
  if(internal::traits<Input>::Layout == RowMajor){
    std::cout << "row major" << std::endl;
  }
  std::cout << "Params:" << std::endl;
  std::cout << "kernelRows: "<< kernelRows << std::endl;
  std::cout << "kernelCols: " << kernelCols << std::endl;
  std::cout << "row_stride: " << row_stride << std::endl;
  std::cout << "col_stride: " << col_stride << std::endl;
  std::cout << "row_in_stride: " << row_in_stride << std::endl;
  std::cout << "col_in_stride: " << col_in_stride << std::endl;
  std::cout << "padding_type: " << padding_type << std::endl;
  std::cout << "kernel_dims[0]: " << kernel_dims[0] << std::endl;
  std::cout << "kernel_dims[1]: " << kernel_dims[1] << std::endl;
  std::cout << std::endl;
  std::cout << "image_patch_out_dim: " << input.dimensions()[0] << " "
            << kernelCols << " " << kernelRows << " "
            << kernelChannels << std::endl;
  std::cout << std::endl;
  std::cout << "pre_contract_dims: " << pre_contract_dims[0] << " "
            << pre_contract_dims[1] << std::endl;
  std::cout << std::endl;
  std::cout << "post_contract_dims: ";
  for(int i=0; i<NumDims; ++i){
    std::cout << post_contract_dims[i] << " ";
  }
  std::cout << std::endl;
  std::cout << std::endl;
  std::cout << "kernel_dims: " << kernel_dims[0] << " "
            << kernel_dims[1] << std::endl;
  std::cout << std::endl;
  std::cout << "array<IndexPair<TensorIndex>, 1> contract_dims: "
            << 1 << " " << 0 << std::endl;
  std::cout << std::endl;
  // auto num_patches = 8;
  // Eigen::Tensor<float, 5, Eigen::RowMajor, int64_t> tensor(input.dimensions()[0],
  //       num_patches, kernelCols, kernelRows, kernelChannels);
  //
  //
  // return input
  //     .extract_image_patches(kernelRows, kernelCols, row_stride, col_stride,
  ...
  // PrintValues<Device,Inter>::print(device,inter);
  // std::cout << typeid(input
  //     .extract_image_patches(kernelRows, kernelCols, row_stride, col_stride,
  //                            row_in_stride, col_in_stride, padding_type)).name()
  //                            << std::endl;
...
const TensorContractionOp<const Eigen::array<Eigen::IndexPair<long>, 1>, const Eigen::TensorReshapingOp<const Eigen::DSizes<long, 2>, const Eigen::TensorImagePatchOp<-1, -1, const Eigen::TensorMap<Eigen::Tensor<const float, 4, 1, long>, 16, MakePointer> > >, const Eigen::TensorReshapingOp<const Eigen::DSizes<long, 2>, const Eigen::TensorMap<Eigen::Tensor<const float, 4, 1, long>, 16, MakePointer> > >
    SpatialConvolutionModified2(const Device& device, const Inter& inter, const Input& input, const Kernel& kernel,
                       const DenseIndex row_stride = 1,
                       const DenseIndex col_stride = 1,
                       const PaddingType padding_type = PADDING_SAME,
                       const DenseIndex row_in_stride = 1,
  ...
  TensorRef<Tensor<typename internal::traits<Kernel>::Scalar,
                   ...
                   internal::traits<Kernel>::Layout, TensorIndex> >
      kern(kernel);
  printf("SpatialConvolutionModified2 called\n");
  EIGEN_STATIC_ASSERT(
      internal::traits<Input>::Layout == internal::traits<Kernel>::Layout,
      YOU_MADE_A_PROGRAMMING_MISTAKE);
  const bool isColMajor = (internal::traits<Input>::Layout == ColMajor);
  ...
  return input
      .extract_image_patches(kernelRows, kernelCols, row_stride, col_stride,
                             row_in_stride, col_in_stride, padding_type).reshape(pre_contract_dims).contract(kernel.reshape(kernel_dims), contract_dims);
  // PrintValues<Device,Inter>::print(device,inter);
  // std::cout << typeid(input
  //     .extract_image_patches(kernelRows, kernelCols, row_stride, col_stride,
  //                            row_in_stride, col_in_stride, padding_type)).name()
