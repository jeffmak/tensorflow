...
def main():
  ...
  compiler_flags = compiler_flags + ['-DTENSORFLOW_USE_SYCL', '-DEIGEN_HAS_C99_MATH']
  ...
  pipe = Popen([CPU_CXX_COMPILER] + flags_without_output + ["-DEIGEN_USE_SYCL=1", "-E"], stdout=PIPE)
  ...
  computecpp_device_compiler_flags = ['-DEIGEN_USE_SYCL=1', '-sycl-compress-name', '-Wno-unused-variable', '-Wno-c++11-narrowing',
  ...
  if x == 0:
    ...
    host_compiler_flags = ['-DEIGEN_USE_SYCL=1', '-xc++', '-Wno-unused-variable', '-I', COMPUTECPP_INCLUDE, '-c', bc_out] + host_compiler_flags
