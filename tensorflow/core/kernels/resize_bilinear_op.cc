...
// // Partial specialization of ResizeBilinear functor for a SYCLDevice.
// namespace functor {
// template <typename T>
// void resize_image_sycl(typename TTypes<T, 4>::ConstTensor images,
//                   const int batch_size, const int64 in_height,
//                   const int64 in_width, const int64 out_height,
//                   const int64 out_width, const int channels,
//                   const Eigen::Tensor<int64, 2>& xs_tensor, const Eigen::Tensor<float, 1>& xs_lerp,
//                   const Eigen::Tensor<int64, 2>& ys_tensor, const Eigen::Tensor<float, 1>& ys_lerp,
//                   // const std::vector<CachedInterpolation>& xs_vec,
//                   // const std::vector<CachedInterpolation>& ys,
//                   typename TTypes<float, 4>::Tensor output) {
//   const int64 in_row_size = in_width * channels;
...
//   void operator()(const SYCLDevice& d, typename TTypes<T, 4>::ConstTensor images,
//                   const float height_scale, const float width_scale,
//                   typename TTypes<float, 4>::Tensor output) {
//     const int batch_size = images.dimension(0);
//     const int64 in_height = images.dimension(1);
...
//     }
//
//     Tensor<int64, 2> ys_tensor(out_height+1, out_height+1);
//     Tensor<float, 1> ys_lerp(out_height+1);
//     Tensor<int64, 2> xs_tensor(out_width+1, out_width+1);
//     Tensor<float, 1> xs_lerp(out_width+1);
//
//     for(int i = 0; i < ys.size(); ++i){
//       ys_tensor(i, 0) = ys[i].lower;
//       ys_tensor(i, 1) = ys[i].upper;
//       ys_lerp(i) = ys[i].lerp;
//     }
//
//     for(int i = 0; i < xs.size(); ++i){
//       xs_tensor(i, 0) = xs[i].lower;
//       xs_tensor(i, 1) = xs[i].upper;
//       xs_lerp(i) = xs[i].lerp;
//     }
//
//     resize_image_sycl<T>(images, batch_size, in_height, in_width, out_height,
//                     out_width, channels, xs_tensor, xs_lerp, ys_tensor, ys_lerp, output);
//   }
// };
// }  // namespace functor
// #endif
