...
struct LaunchMaxPoolingGradOpSYCL {
  static void launch(OpKernelContext* context, const Tensor& tensor_in,
    ...
    std::array<int64, 3> input_size{{tensor_in.dim_size(2),
    ...
    for (int64 r = 0; r < out_backprop.dim_size(2); ++r) {
      ...
      for (int64 c = 0; c < out_backprop.dim_size(1); ++c) {
        ...
        src.FillIndicesAndSizes<4>(out_backprop.shape(), &src_indices,
                                   &src_sizes);
        dst.FillIndicesAndSizes<4>(tensor_in.shape(), &dst_indices,
                                   &dst_sizes);
        ...
        auto tensor_in_slice =
            tensor_in.tensor<T, 4>().slice(dst_indices, dst_sizes);

        // Slice from tensor_out.
        auto tensor_out_slice =
            tensor_out.tensor<T, 4>().slice(src_indices, src_sizes);

        // Backprop slice.
        auto out_backprop_slice =
        ...
        Eigen::Tensor<T, 4, Eigen::RowMajor> mat0(dst_sizes);
        // mat0.setZero();
        auto select_slice =
        ...
        output->tensor<T, 4>()
            ...
            .device(context->eigen_sycl_device()) += select_slice;
      }
    }
  }
};
